...
It is **crucial** to set the correct values in the `net_config.h` file for the library to work properly. Pay special attention to the following parameters:
...
- **`WCHNET_NUM_TCP_LISTEN`**: Set this to the number of server listeners (for example, for one server, set `WCHNET_NUM_TCP_LISTEN = 1`). Each listening socket requres an additional TCP socket (i.e. increase WCHNET_NUM_TCP by 1)